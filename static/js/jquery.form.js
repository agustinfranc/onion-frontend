...
*/
  ...
  $.fn.ajaxSubmit = function (options) {
    ...
    const elements = []
    let qx
    const a = this.formToArray(options.semantic, elements)
    if (options.data) {
    ...
    if (options.type.toUpperCase() == 'GET') {
      options.url += (options.url.includes('?') ? '&' : '?') + q
    ...
    function fileUploadIframe(a) {
          ...
          if (scr || s.textarea) {
            ...
            if (ta) {
              ...
              if (pre) {
                ...
                xhr.responseText = b.textContent ? b.textContent : b.textContent
              }
            }
      ...
      var httpData = function (xhr, type, s) {
        ...
        const xml = type === 'xml' || (!type && ct.includes('xml'))
        ...
        if (typeof data === 'string') {
          if (type === 'json' || (!type && ct.includes('json'))) {
            data = parseJSON(data)
          } else if (
            type === 'script' ||
            (!type && ct.includes('javascript'))
          ) {
            $.globalEval(data)
          }
        }
